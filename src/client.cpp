...
Rcpp::RawVector robust_grpc_client_call( Rcpp::CharacterVector r_target_str,
    ...
    if (request_bb) grpc_byte_buffer_destroy(request_bb);
    if (response_bb) grpc_byte_buffer_destroy(response_bb);
    grpc_slice_unref(details_slice_recv);

    // Unref metadata slices from metadata_store
    for (const auto& meta : metadata_store) {
        grpc_slice_unref(meta.key);
        grpc_slice_unref(meta.value);
    }
    grpc_metadata_array_destroy(&initial_metadata_send); // The array itself
    grpc_metadata_array_destroy(&initial_metadata_recv);
    grpc_metadata_array_destroy(&trailing_metadata_recv);

    if (call) grpc_call_unref(call); // CRITICAL: Was missing in nfultz/grpc
    ...
    grpc_shutdown(); // Should ideally be managed globally
    RGRPC_LOG("Robust Client: Fetch function complete.");

    // If RPC failed, throw an error to R. Otherwise, return the result.
    if (status_code_recv != GRPC_STATUS_OK && result_rawvector.length() == 0) { // Check length in case of prior partial success
        Rcpp::stop(error_detail_str);
    }
    return result_rawvector;
}

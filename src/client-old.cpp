...
RawVector fetch(CharacterVector server, CharacterVector method, RawVector requestArg, CharacterVector metadata) {
  ...
  // --- START: ADD GRPC TRACING (Client-side) ---
  RGRPC_LOG("Enabling gRPC C-core tracing for client (verbosity line commented out)...");
  // gpr_set_log_verbosity(GPR_LOG_SEVERITY_DEBUG); // <<<< COMMENTED OUT / REMOVED

  const char* client_tracers = "all";
  grpc_tracer_set_enabled(client_tracers, true);
  RGRPC_LOG("gRPC Client Tracers enabled for: " << client_tracers);
  // --- END: ADD GRPC TRACING (Client-side) ---

  // gpr_timespec deadline = five_seconds_from_now();


  RGRPC_LOG("Initializing gRPC core (client-side, if not already)...");
  grpc_init();
  ...
  grpc_channel *channel = grpc_channel_create(Rcpp::as<std::string>(server[0]).c_str(), NULL, NULL);
  ...
  for(int i = 0; i < metadata_length; i++) {
    ...
    // Assuming other members like 'flags' and the internal 'reserved' union
    // are okay to be left uninitialized or will be zero-initialized by default
    // when the array 'meta_c' was declared, or are not used before being set later.
    // If you encountered further errors related to these, you might need to explicitly set them:
    // meta_c[i].flags = 0;
    // memset(&meta_c[i].reserved, 0, sizeof(meta_c[i].reserved));
  }
